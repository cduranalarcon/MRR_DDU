#######################################################################################################
#######################################################################################################
## set of MRR function for python3
##
...
# check for the special character '~' a,d ':' that stops the IMProToo routine
def check_RawFiles(filename, file_out):
    ...
    dataset = open(filename,"r") # Open existing raw file
    dataset0 = open(file_out,"w")# Create empty file 
    
    n_errors = 0
    
    for i in np.linspace(1,650000,650000): #A simple do loop and readlines() have problem to read all the line of the raw files. 650000 lines is large enough for a full day of data.
        #try:            
        line = dataset.readline() # Read each line
        #except:
        #    print('Error in line = ', i,line) 

        if (np.size(line.split('~'))==1) & (np.size(line.split(':'))==1): #bad lines in the raw files may have a ':' or '~'. This excludes both cases.

            if line[:3] == 'MRR': #Detects the first line of a sample
                sample = ''
                nlines_sample = 0

            sample = sample + line #joins all the lines of a sample.
            nlines_sample = nlines_sample + 1 # counts the number of lines in a sample 
            
            if ((line[:3] == 'F63') & (nlines_sample == 67)): # The last line of the sample is the gate 'F63' and it should be the line number 67
                dataset0.write(sample) #Write the full sample in the new raw file
...
def raw2snow(file_in,file_out, TRES = 60, Descr = "MRR data", author = "C. Duran-Alarcon, IGE - UGA",ncForm="NETCDF3_CLASSIC"): #convert raw files in MRR Doppler moments
    ...
    sys.path.append(os.getcwd()+path_IMProToo)
    import core3 as IMProToo # Minimum width of a peak set as 4 bins(instead of 3) to remove clutter at the expense of sensitivity      
